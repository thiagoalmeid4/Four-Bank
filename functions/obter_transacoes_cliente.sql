        ...
        SELECT
            ...
            CASE
                WHEN t.fk_nr_id_conta_destino = c.nr_id_conta THEN
                    (SELECT nm_cliente FROM tb_conta INNER JOIN tb_cliente ON tb_conta.fk_nr_id_cliente = tb_cliente.nr_id_cliente WHERE nr_id_conta = t.fk_nr_id_conta_origem)::character varying
                WHEN t.fk_nr_id_conta_origem = c.nr_id_conta THEN
                    (SELECT nm_cliente FROM tb_conta INNER JOIN tb_cliente ON tb_conta.fk_nr_id_cliente = tb_cliente.nr_id_cliente WHERE nr_id_conta = t.fk_nr_id_conta_destino)::character varying
